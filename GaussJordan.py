...
def GaussJordan(matrix) : # Defines a function named GaussJordan that takes one argument matrix.
    ...
    for i in range(n) : # Starts a loop that will iterate n times. The variable i will take the values from 0 to n - 1 in each iteration.
        ...
        for k in range(i + 1, n) :
            ...
            # Explanation: Multiply row {i} by {c} and add it to row {k} to make the first column zero below row {i}.
...
def main() :
    ...
    try :
        ...
        # Checks if the solution is a string (example : an error message). If so, prints the error message. Otherwise, prints the solution.
